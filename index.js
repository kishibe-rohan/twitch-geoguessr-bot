...
countries.forEach((country) => (country.count = 0));
...
client.on("chat", (channel, tags, message, self) => {
  ...
  if (counting) getGuess(message);

  if (message === "!start") {
    counting = true;
    // if (user["display-name"] === "rogueeeeeeeee1")
  }
...
const getGuess = (message) => {
  //console.log("Counting..");
  if (message.startsWith("!guess")) {
    message = message.replace("!guess", "");
    message = message.trim();
    // console.log(message.toUpperCase());
    countries.forEach((country) => {
      if (message.toUpperCase() === country["alpha-3"]) {
        country.count += 1;
        guessCount += 1;
      }
    });
    ...
    countries.forEach((country) => {
      if (country.count > topCount) {
        topCount = country.count;
        topGuess = country.name;
      }
    });

    var counts = countries.sort((a, b) =>
